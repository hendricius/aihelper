...
struct ContentView: View {
    ...
    var body: some View {
        VStack(spacing: 12) {
                ...
                VStack(spacing: 6) {
                    if !permissionManager.hasAccessibilityPermission {
                        HStack {
                            ...
                            Text("Hotkeys & Hyper Key need Accessibility + Input Monitoring")
                                .font(.caption)
                            Spacer()
                            Button("Fix") {
                                // Prompt for both permissions the Hyper Key/hotkeys need and
                                // arm the Hyper Key automatically once they're granted.
                                HyperKeyManager.shared.enableRequestingPermissions()
                    ...
                    HStack {
                        ...
                        Button("Recheck All") {
                            ...
                            // Re-register shortcuts and arm the Hyper Key if accessibility
                            // (and input monitoring) are now granted.
                            if permissionManager.hasAccessibilityPermission {
                                (NSApp.delegate as? AppDelegate)?.registerShortcuts()
                            }
                            HyperKeyManager.shared.armIfReady()
                        }
                        .font(.caption)
            ...
            // Development machines (hosts from ~/.ssh/config)
            if !devMachinesViewModel.hosts.isEmpty {
                Divider()
                DevelopmentMachinesInlineView(viewModel: devMachinesViewModel)
            }
        }
        .padding()
    ...
    private var calculatedHeight: CGFloat {
        ...
        // Add height for the inline Development Machines list
        if !devMachinesViewModel.hosts.isEmpty {
            let listHeight = min(CGFloat(devMachinesViewModel.hosts.count) * 36, 200)
            height += listHeight + 44  // header row + divider + spacing
        }
...
// MARK: - Keep-Awake (Caffeine) Bar

struct CaffeineStatusBar: View {
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                Text(caffeine.isActive ? "Keeping Mac awake" : "Keep Mac awake")
                    .font(.caption)
                    .fontWeight(.medium)

                if caffeine.isActive {
                    Text("Screen won't lock · \(caffeine.remainingText) left")
                        .font(.caption2.monospacedDigit())
                        .foregroundColor(.secondary)
                } else {
                    Text("Stay awake & keep the screen unlocked")
                        .font(.caption2)
                        .foregroundColor(.secondary)
                }
            }
            ...
            // Duration picker (locked while active — stop first to change it)
            Menu("\(caffeine.durationHours)h") {
                ForEach(CaffeineManager.minHours...CaffeineManager.maxHours, id: \.self) { hours in
                    Button("\(hours) hour\(hours == 1 ? "" : "s")") { caffeine.durationHours = hours }
                }
            }
            .font(.caption2)
            ...
            .help("How long to stay awake")

            Toggle("", isOn: Binding(get: { caffeine.isActive }, set: { _ in caffeine.toggle() }))
                .toggleStyle(.switch)
                .labelsHidden()
                .scaleEffect(0.8)
        ...
        .padding(.horizontal, 12)
        .padding(.vertical, 8)
        .background(caffeine.isActive ? Color.orange.opacity(0.1) : Color.gray.opacity(0.1))
        .cornerRadius(8)
    }
}
